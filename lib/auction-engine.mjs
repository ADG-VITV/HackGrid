 ...
 *
 * The event runs strictly one capsule at a time, in catalogue order. Pods for a
 * capsule are created at the moment that capsule opens — never earlier — so
 * landing on the bidding page costs one small query instead of five rounds'
 * worth of pod assignments.
 */
...
export async function ensureEvent(prisma) {
  const event = await prisma.event.upsert({
    where: { key: EVENT_KEY },
    update: {},
    create: { key: EVENT_KEY, name: "HackGrid", startingBudget: 1000 },
  });
...
export async function getBiddingContext(prisma, teamIdOrEmail) {
  ...
  const owned = settlements
    .map((s) => ({
      ...
      sequenceOrder: s.capsule.sequenceOrder ?? capsuleOrder.indexOf(s.capsule.key) + 1,
