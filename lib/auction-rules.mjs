 ...
 *   - after a bid lands, the tier closes 10s later unless someone raises
 */
...
export const BID_TIMEOUT_SECONDS = seconds("HACKGRID_BID_TIMEOUT_SECONDS", 10);
...
/** Breather between a capsule finishing and the next one opening. */
export const CAPSULE_HANDOVER_SECONDS = seconds("HACKGRID_HANDOVER_SECONDS", 20);
 ...
 * holds until all but one of the pod is present. One short is deliberate: a
 * single missing team must not be able to stall the whole round.
 */
export function quorumFor(podSize) {
  return Math.max(1, podSize - 1);
}
