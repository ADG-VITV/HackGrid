...
export function createAuctionHub({ connectionString, io }) {
  ...
  function personalise(snapshot, teamId) {
    ...
    return {
      ...
      you: {
        ...
        remainingBalance: STARTING_BALANCE - (snapshot.spentByTeam.get(teamId) ?? 0),
  ...
  async function autoAssign(podId, lot) {
    ...
    if (remaining.length !== 1) {
      ...
      devLog(
        podId,
        `Skipped auto-assign for "${lot.subCapsule.name}": ${remaining.length} teams unassigned, expected exactly 1.`,
        "warn",
      );
      return;
    }
  ...
  async function closeLot(lotId) {
    return withLotLock(lotId, async () => {
      ...
      if (!topBid) {
        // Nobody bid inside the window. Nothing sells; move on.
        ...
        devLog(
          podId,
          isRemainder
            ? `"${lot.subCapsule.name}" went unclaimed.`
            : `"${lot.subCapsule.name}" closed with no bids.`,
          "warn",
        );
   ...
   * settled the capsule closes and the next one opens - and the pods for that
   * next capsule are created right here, not before, which is why landing on
   * the bidding page stays cheap however far into the event you are.
   */
  async function afterLotSettled(capsuleId) {
    ...
    try {
      if (await mainPodsFinished(prisma, capsuleId)) {
        ...
        if (pendingRemainder > 0) {
          ...
          if (opened) {
            ...
            const startedRemainder = await maybeStartTimers(opened.podId);
            if (!startedRemainder) await pushRoomState(opened.podId);
            ensureTicking();
            log(`capsule ${capsuleId}: remainder pod opened with ${opened.lotIds.length} tier(s)`);
            return;
          }
        }
      }
      ...
      broadcastAll("CAPSULE_CLOSED", { capsuleId });
      ...
      // A short breather so teams can read their results before the next round.
      const timer = setTimeout(() => {
        void startNextCapsule(nextKey);
      }, CAPSULE_HANDOVER_SECONDS * 1000);
      if (typeof timer.unref === "function") timer.unref();

      broadcastAll("CAPSULE_HANDOVER", {
        nextKey,
        opensAt: new Date(Date.now() + CAPSULE_HANDOVER_SECONDS * 1000).toISOString(),
      });
    } catch (error) {
  ...
  async function startNextCapsule(capsuleKey) {
    try {
      const report = await startCapsule(prisma, capsuleKey, { force: true });
      if (report.status !== "success") {
        log(`could not open ${capsuleKey}: ${report.message}`);
        return;
      }
      log(report.message);
      await onCapsuleStarted(report.capsuleId);
      broadcastAll("CAPSULE_OPENED", { capsuleKey, capsuleId: report.capsuleId });
    } catch (error) {
      log("startNextCapsule failed", error?.message);
    }
  }
   ...
   */
  async function attach(socket, { podId, teamId }) {
    const membership = await prisma.podMembership.findUnique({
  ...
  return { prisma, attach, detach, handleBid, pushRoomState, onCapsuleStarted, hydrate };
}
