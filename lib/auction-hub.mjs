...
export function createAuctionHub({ connectionString, io }) {
  ...
  async function afterLotSettled(capsuleId) {
    ...
    try {
      ...
      // (Force in development, the admin portal in production) to draw its pods.
      log(`waiting for the organiser to open ${nextKey}`);
    } catch (error) {
  ...
  return { prisma, attach, detach, handleBid, pushRoomState, onCapsuleStarted, broadcastAll, hydrate };
}
